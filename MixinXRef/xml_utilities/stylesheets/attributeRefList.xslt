...
<xsl:template name="attributeRefListTable">
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="attributeTable display">
			...
			<tfoot>
				<tr>
					<td><xsl:value-of select="count( distinct-values( /MixinXRefReport/Attributes/Attribute[ ru:contains($attributeRefs/@ref, @id) ]/@namespace ) )" /></td>
					<td><xsl:value-of select="count( distinct-values( /MixinXRefReport/Attributes/Attribute[ ru:contains($attributeRefs/@ref, @id) ]/@id ) )" /></td>
					<td>-</td>
					<td><xsl:value-of select="count( distinct-values( /MixinXRefReport/Attributes/Attribute[ ru:contains($attributeRefs/@ref, @id) ]/@assembly-ref ) )" /></td>
					<td>-</td>
				</tr>
			</tfoot>
			<tbody>
          ...
          <xsl:variable name="attr" select="key('attribute', @ref)"/>
					<tr>
						<td><xsl:value-of select="$attr/@namespace"/></td>
						<td>
						...
						<td><xsl:value-of select="count( $attr/AppliedTo/InvolvedType-Reference )" /></td>
						<td>
